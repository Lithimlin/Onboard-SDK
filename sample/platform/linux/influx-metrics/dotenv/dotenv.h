...
parse(const std::string& src)
{
  ...
  for (std::sregex_iterator i = line_begin; i != line_end; ++i)
  {
        ...
        while ((pos = value.find("\\", pos)) != std::string::npos)
        {
          if (pos + 1 < value.size())
          {
                ...
                c == '?' || c == '\'')
            {
              if (c == 'a')
              {
                value.replace(pos, 2, "\a");
              }
              else if (c == 'b')
              {
                value.replace(pos, 2, "\b");
              }
              else if (c == 'f')
              {
                value.replace(pos, 2, "\f");
              }
              else if (c == 'n')
              {
                value.replace(pos, 2, "\n");
    ...
    {
      // If the value is an empty string, set the variable in the environment
          ...
          if (end != std::string::npos)
          {
            ...
            else
            {
              ...
              std::string default_value =
                key_with_default.substr(colon_pos + 1);
...
load(const std::string& path)
{
  ...
  if (file.is_open())
  {
    ...
    for (const auto& pair : map)
    {
      ...
      if (value.empty())
      {
        // If the value is an empty string, set it to the default value if
        // available
